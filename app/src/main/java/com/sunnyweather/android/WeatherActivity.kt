...
import com.sunnyweather.android.ui.WeatherViewModel
...
class WeatherActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.nowWeather.navBtn.setOnClickListener {
    ...
    private fun showWeatherInfo(weather: Weather){
        binding.nowWeather.placeName.text = viewModel.placeName
        val realtime = weather.realtime
        val daily = weather.daily
        val currentTempText = "${realtime.temperature.toInt()} ℃"
        binding.nowWeather.currentTemp.text = currentTempText
        binding.nowWeather.currentSky.text = getSky(realtime.skycon).info
        val currentPM25Text = "空气指数 ${realtime.airQuality.aqi.chn.toInt()}"
        binding.nowWeather.currentAQI.text = currentPM25Text
        binding.nowWeather.nowLayout.setBackgroundResource(getSky(realtime.skycon).bg)
        binding.forecastWeather.forecastLayout.removeAllViews()
        val days = daily.skycon.size
        for(i in 0 until days){
            val skycon = daily.skycon[i]
            val temperature = daily.temperature[i]
            val view = LayoutInflater.from(this).inflate(R.layout.forecast_item, binding.forecastWeather.forecastLayout, false)
            ...
            binding.forecastWeather.forecastLayout.addView(view)
        }
